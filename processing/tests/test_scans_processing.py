...
def test_return_cached_percentage_full_match(test_db_path):
    ...
    The function tests the following:
    1. When all CVEs in the set are found in the database, the
...
def test_return_cached_percentage_partial_match(test_db_path):
    ...
    The function tests the following:
    1. When some CVEs in the set are found in the database, the
    function returns correct percentage.
...
def test_return_cached_percentage_no_match(test_db_path):
    ...
    The function tests the following:
    1. When no CVEs in the set are found in the database,
...
def test_return_cached_percentage_custom_chunk_size(test_db_path):
    ...
    The function tests the following:
    1. The function correctly calculates the percentage when
    using a smaller chunk size.
    """
    cve_set = {"CVE-2021-1234", "CVE-2022-5678", "CVE-2022-1111"}

    percentage = return_cached_percentage(
        cve_set, chunk_size=1, db_file=test_db_path
    )
    assert percentage == 100.0  # Test 1.
