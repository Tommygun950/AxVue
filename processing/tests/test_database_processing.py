...
def test_queried_nvd_data_feed_table(temp_db_file):
    ...
    cursor.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='queried_nvd_data_feed';")
    ...
    assert len(columns) == 4, f"Expected 4 columns in queried_nvd_data_feed table, got {len(columns)}" # Test 1.

    pk_column = next((col for col in columns if col[5] == 1), None)
    assert pk_column is not None, "No primary key defined for queried_nvd_data_feed table" # Test 2.
    ...
    for expected_col in expected_columns:
        assert expected_col in column_names, f"""
            Column '{expected_col}' missing from queried_nvd_data_feed table
        """ # Test 4.
        col_index = column_names.index(expected_col)
        col_type = columns[col_index][2]
        assert col_type == expected_types[expected_col], f"""
...
def test_stored_nvd_data_feed_table(temp_db_file):
    ...
    cursor.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='stored_nvd_data_feed';")
    ...
    assert len(columns) == 6, f"Expected 6 columns in stored_nvd_data_feed table, got {len(columns)}" # Test 1.

    pk_column = next((col for col in columns if col[5] == 1), None)
    assert pk_column is not None, "No primary key defined for stored_nvd_data_feed table" # Test 2.
    ...
    for expected_col in expected_columns:
        assert expected_col in column_names, f"""
            Column '{expected_col}' missing from stored_nvd_data_feed table
        """ # Test 4.
        col_index = column_names.index(expected_col)
        col_type = columns[col_index][2]
        assert col_type == expected_types[expected_col], f"""
    ...
    required_columns = ['file_byte_size', 'cve_count', 'last_updated', 'status', 'is_complete']
    for required_col in required_columns:
        assert f"{required_col} {expected_types[required_col]} NOT NULL" in create_stmt, f"""
            NOT NULL constraint missing for required column
             '{required_col}' in CREATE TABLE statement
