...
def _add_scan(scan_name: str, file_path: str, db_file: str = "vuln_data.db") -> tuple[bool, str]:
    """
    ...
    if scan_name == "" or file_path == "":        
        error_dialog = GeneralErrorDialog("Scan name or file path cannot be empty.")
        error_dialog.exec_()
...
def _edit_scan(scan_id: int, scan_name: str, file_path: str, cache_enabled: bool, db_file: str = "vuln_data.db") -> tuple[bool, str]:
    """
...
def _delete_scan(scan_id: int, db_file: str = "vuln_data.db") -> tuple[bool, str]:
    """
...
def _update_scan_selected_status(scan_id: int, selected: bool, db_file: str = "vuln_data.db") -> tuple[bool, str]:
    """
    ...
    try:
        ...
        cursor.execute("UPDATE scan_data SET selected = ? WHERE id = ?", (selected_int, scan_id))
        
        conn.commit()
        conn.close()
        
        return (True, f"Successfully updated selection status for scan ID: {scan_id}")
...
def _get_all_scan_data(db_file: str = "vuln_data.db") -> list[dict]:
    """
...
def _get_scan_data(scan_id: int, db_file: str = "vuln_data.db") -> dict:
    """
    ...
    try:
        ...
        cursor.execute("SELECT scan_name, file_path, total_vulnerabilities, unique_cve_list, cache_enabled, cached_percentage FROM scan_data WHERE id = ?", (scan_id,))
        ...
        if scan_data:
            ...
            error_dialog = GeneralErrorDialog(f"Scan with ID {scan_id} not found.", None)
            error_dialog.exec_()
            return None
